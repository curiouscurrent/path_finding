...
if uploaded_file:
    ...
    # Check if trash was found
    if not trash_coords:
        st.write("No waste found in the uploaded image.")
    else:
        ...
        def fitness_function(positions):
            ...
            for idx, agent_position in enumerate(positions):
                closest_trash = min(
                    range(len(trash_coords)),
                    key=lambda x: np.linalg.norm(
                        np.array(agent_position) - np.array(trash_coords[x])
                    ),
                )
                total_distance += np.linalg.norm(
                    np.array(agent_position) - np.array(trash_coords[closest_trash])
                )
        ...
        def dynamic_path_recalculation(trash_coords, num_agents):
            ...
            while len(collected_trash) < len(trash_coords):
                for agent_idx in range(num_agents):
                    ...
                    remaining_trash = [
                        i for i in range(len(trash_coords)) if i not in collected_trash
                    ]
                    ...
                    if len(remaining_trash) == 1:
                        ...
                        for other_agent_idx in range(num_agents):
                            if other_agent_idx != agent_idx:
                                ...
                                distance_between_agents = euclidean(
                                    agent_position, other_agent_position
                                )
                                if distance_between_agents < repulsion_distance:
                                    ...
                                    repulsion_direction = (
                                        ...
                                        - np.array(other_agent_position)
                                    )
                                    repulsion_force += repulsion_direction / (
                                        distance_between_agents + 1e-6
                                    )  # Avoid division by zero

                        # Apply both attractive and repulsive forces
                        force = (
                            attraction_strength * direction_to_goal
                            + repulsion_strength * repulsion_force
                        )
                        ...
                        closest_trash = min(
                            remaining_trash,
                            key=lambda x: euclidean(agent_position, trash_coords[x]),
                        )
                        goal_position = trash_coords[closest_trash]
